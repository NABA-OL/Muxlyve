...
export async function checkLicense({ isPackaged }) {
  if (!isPackaged && process.env.MS_FORCE_LICENSE !== '1') return { unlocked: true, reason: 'dev' };
  if (!isPackaged && process.env.MS_DEV_UNLOCK === '1') return { unlocked: true, reason: 'env' };
  ...
  try {
    ...
    if (stored.plan === 'lifetime') {
      return { unlocked: true, reason: 'offline-lifetime', email: stored.email };
    }
...
export async function activateLicense(key) {
  try {
    ...
    console.error('[license] activate error:', err.status, err.message, err.data);
    ...
    if (err.status === 402) return { ok: false, error: 'Tu suscripción fue cancelada o venció. Renuévala para activar Muxlyve.' };
    const errBody = (err.data?.error || err.data?.message || err.message || '').toLowerCase();
    if (errBody.includes('cancel') || errBody.includes('inactive') || errBody.includes('suspend') || errBody.includes('expired')) {
      return { ok: false, error: 'Tu suscripción fue cancelada o venció. Renuévala para activar Muxlyve.' };
    }
    if (err.status >= 400 && err.status < 500) return { ok: false, error: 'Clave inválida o suscripción inactiva.' };
